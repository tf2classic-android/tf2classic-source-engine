...
void CTFBotTacticalMonitor::AvoidBumpingEnemies( CTFBot *actor )
{
	if ( actor->m_iSkill > CTFBot::NORMAL )
	{
		...
		for ( int i=0; i<enemies.Count(); ++i )
		{
			...
			if ( pPlayer->m_Shared.InCond( TF_COND_STEALTHED ) || pPlayer->m_Shared.InCond( TF_COND_DISGUISED ) )
				continue;
...
CObjectTeleporter *CTFBotTacticalMonitor::FindNearbyTeleporter( CTFBot *actor )
{
	...
	for ( int i=0; i<objects.Count(); ++i )
	{
		...
		if ( pTele->ObjectType() != OBJ_TELEPORTER || !(pTele->GetState() == TELEPORTER_STATE_READY) || pTele->GetLastKnownArea() == nullptr || nearby.IsEmpty() )
			continue;
...
void CTFBotTacticalMonitor::MonitorArmedStickybombs( CTFBot *actor )
{
	...
	for ( CTFGrenadeStickybombProjectile *pGrenade : pLauncher->m_Pipebombs )
	{
