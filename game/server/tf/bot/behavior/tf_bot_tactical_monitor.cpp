...
void CTFBotTacticalMonitor::MonitorArmedStickyBombs( CTFBot *me )
{
	if ( m_stickyBombCheckTimer.IsElapsed() )
	{
		...
		if ( gun )
		{
			const CUtlVector< CHandle< CTFGrenadeStickybombProjectile > > &pipeBombVector = gun->GetPipeBombVector();
			...
			if ( pipeBombVector.Count() > 0 )
			{
				...
				{
					CTFGrenadeStickybombProjectile *sticky = pipeBombVector[p];
					if ( !sticky )
					{
						continue;
					}
...
CObjectTeleporter *CTFBotTacticalMonitor::FindNearbyTeleporter( CTFBot *me )
{
	...
	for( int j=0; j<objVector.Count(); ++j )
	{
		if ( objVector[j]->GetType() == OBJ_TELEPORTER )
		{
			...
			if( (teleporter->GetType() == TELEPORTER_TYPE_ENTRANCE) && (teleporter->GetState() == TELEPORTER_STATE_READY) && teleporterArea )
			{
