...
bool CTFBotLocomotion::IsAreaTraversable( const CNavArea *baseArea ) const
{
	...
	{
		switch ( GetBot()->GetEntity()->GetTeamNumber() )
		{
			case TF_TEAM_RED:
			{
	...
	{
		if ( TFGameRules()->GetWinningTeam() != GetBot()->GetEntity()->GetTeamNumber() )
			return false;
	}
...
bool CTFBotLocomotion::IsEntityTraversable( CBaseEntity *ent, TraverseWhenType when ) const
{
	if ( ent )
	{
		...
		if ( ent->IsBaseObject() )
		{
			...
			if ( pObject->GetBuilder() == GetBot()->GetEntity() ) // we can't walk through our own buildings...
				return false;
		}
	}
...
bool CTFBotVision::IsIgnored( CBaseEntity *ent ) const
{
	...
	{
		CBaseObject *pObject = static_cast<CBaseObject *>( ent );
		if ( pObject->IsPlacing() || pObject->HasSapper())
			return true;
	}
